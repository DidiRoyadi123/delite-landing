<?php
include_once "../sambungan.php";

$user = $_POST['username'];
$pass = md5($_POST['password']);
$sql = "SELECT * FROM karyawan WHERE username='$user' AND password='$pass' AND aktif='Y'";
$login=mysqli_query($koneksi,$sql);
$ketemu=mysqli_num_rows($login);
$b=mysqli_fetch_array($login);
if($ketemu>0){
	session_start();
	$_SESSION['idskasis'] 		= $b['nis'];
	$_SESSION['userskasis']		= $b['username'];
	$_SESSION['namaskasis']		= $b['nama'];
	$_SESSION['emailskasis'] 	= $b['email'];
	if($b['foto']==""){
		$_SESSION['fotoskasis'] = "0.jpg";
	}else{
		$_SESSION['fotoskasis'] = $b['foto'];
	}
	$iddivisi=$b['iddivisi'];
	$sql2 = "SELECT * FROM divisi WHERE iddivisi='$iddivisi'";
	$aksi=mysqli_query($koneksi,$sql2);
	$c=mysqli_fetch_array($aksi);
	$_SESSION['divisiskasis'] 	= $c['divisi'];
	header('location: index.php?m=awal');
}else{
	include "login.php";
	echo '<script language="javascript">';
		echo 'alert ("Username/Password ada yang salah, atau akun anda belum Aktif ,Jika ada kendala bisa hubungi ADMIN")';
	echo '</script>';
}
?>
